...
int main() 
{
   ...
   }
   {
      ...
      if (!testValue 
          (&ctxt, OSUINT32_MAX, encoding, sizeof(encoding))) errcnt++;
   }
   {
      const OSOCTET encoding[] = { 0x1b, 0x00, 0x00, 0x00, 0x01, 
                                   0x00, 0x00, 0x00, 0x00 };
      if (!testValue 
          (&ctxt, ((OSUINT64)OSUINT32_MAX)+1, encoding, sizeof(encoding))) 
         errcnt++;
   }
   {
      const OSOCTET encoding[] = { 0x1b, 0xff, 0xff, 0xff, 0xff, 
                                   0xff, 0xff, 0xff, 0xff };
      if (!testValue (&ctxt, OSUINT64MAX, encoding, sizeof(encoding))) errcnt++;
   }
